...
const DEFAULT_CLIENTS = [
  { id: 'h1', name: 'Example Client', style: 'Knotless braids', amount: 1200, date: '2026-05-20', start_time: '10:00', end_time: '14:00', deposit_paid: true, contact: '@example', notes: 'Waist length, dark brown', created_at: new Date().toISOString() },
]
...
export default function HairClientsPage() {
  const [clients, setClients] = useState(() => JSON.parse(localStorage.getItem('h_hair_clients') || JSON.stringify(DEFAULT_CLIENTS)))
  const [modal, setModal] = useState(null)
  const [form, setForm] = useState({})
  const [filter, setFilter] = useState('upcoming')

  const saveClients = (c) => { setClients(c); localStorage.setItem('h_hair_clients', JSON.stringify(c)) }

  const openAdd = () => {
    const today = new Date().toISOString().slice(0, 10)
    setForm({ date: today, start_time: '10:00', end_time: '14:00', deposit_paid: false, style: STYLES[0], amount: '', name: '', contact: '', notes: '' })
    setModal({ mode: 'add' })
  }
  ...
  const save = () => {
    ...
    if (modal.mode === 'add') saveClients([...clients, { id: 'h' + Date.now(), ...form, amount: Number(form.amount) || 0, created_at: new Date().toISOString() }])
    else saveClients(clients.map(c => c.id === modal.client.id ? { ...c, ...form, amount: Number(form.amount) } : c))
    setModal(null)
  }
  ...
  return (
    <div>
      ...
      <div className="page-sub">Track bookings, reminders 2 days before, sync to calendar</div>
      ...
      {clients.filter(c => { const d = daysUntil(c.date); return d >= 0 && d <= 2 }).map(c => (
        <div key={c.id} className="warn-card">
          <span>⏰</span>
          <div><strong style={{ color: 'var(--flame)' }}>{daysUntil(c.date) === 0 ? 'TODAY' : daysUntil(c.date) === 1 ? 'TOMORROW' : 'IN 2 DAYS'}:</strong> {c.name} — {c.style} — {c.amount}kr {!c.deposit_paid && <span className="deposit-badge deposit-pending" style={{ marginLeft: 6 }}>Deposit pending</span>}</div>
        </div>
      ))}
          ...
          <div className="modal">
            ...
            <div className="form-row">
              ...
              <div className="form-group"><label className="form-label">Contact</label>
                <input value={form.contact || ''} onChange={e => setForm({ ...form, contact: e.target.value })} placeholder="Phone / Instagram" />
              </div>
            </div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">Style</label>
                <select value={form.style || STYLES[0]} onChange={e => setForm({ ...form, style: e.target.value })}>
                  {STYLES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              <div className="form-group"><label className="form-label">Amount (kr)</label>
                <input type="number" value={form.amount || ''} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder="1200" />
              </div>
            </div>
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text2)' }}>Yes, deposit received</span>
                </div>
              </div>
            </div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">Start time</label>
                <input value={form.start_time || ''} onChange={e => setForm({ ...form, start_time: e.target.value })} placeholder="10:00" />
              </div>
              <div className="form-group"><label className="form-label">End time</label>
                <input value={form.end_time || ''} onChange={e => setForm({ ...form, end_time: e.target.value })} placeholder="14:00" />
              </div>
            </div>
            <div className="form-group"><label className="form-label">Notes</label>
              <textarea value={form.notes || ''} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Hair length, colour, special requests..." />
            </div>
            <div className="info-card" style={{ fontSize: 12, color: 'var(--text2)', padding: '10px 14px', marginTop: 8 }}>
              📅 After saving, tap "Add to Calendar" on the booking card to sync to Google Calendar with a 2-day reminder automatically set.
            </div>
